...
/**
 * Sends an HTTP response with standard security and caching headers.
...
/**
 * Safely resolves a request URL to an absolute path inside PUBLIC_DIR.
 * @param {string} urlPath
 * @returns {string} Absolute file path or empty string if invalid
 */
function resolvePublicPath(urlPath) {
  const decoded = decodeURIComponent((urlPath || '/').split('?')[0]);
  let relative = decoded === '/' || decoded === '/settings' ? '/settings.html' : decoded;
...
/**
 * Reads request body stream into a string with a 1MB safety limit.
...
/**
 * Handles the POST /api/settings submission.
 * @param {import('http').IncomingMessage} req
 * @param {import('http').ServerResponse} res
 */
async function handleSettingsApi(req, res) {
  ...
  const fullName = String(payload.fullName !== undefined ? payload.fullName : payload.name || '').trim();
  const email = String(payload.email || '').trim();
  const phone = String(payload.phone !== undefined ? payload.phone : payload.phoneNumber || '').trim();
...
if (require.main === module) {
  ...
  server.listen(PORT, () => {
    console.log(`Settings form server running at http://localhost:${PORT}/settings`);
  });
}
